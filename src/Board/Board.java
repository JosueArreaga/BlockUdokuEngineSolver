...
public class Board implements Cloneable {
    ...
    public void setMove(Piece piece, Piece piece2, Piece piece3) {
        ...
        ArrayList<Board> finalResults = new ArrayList<>();
        ...
        if (isOneLegal && isTwoLegal && isThreeLegal) {
            score1 = bestBoards.get(0).evaluatePosition() + offset;
            score2 = bestBoards.get(1).evaluatePosition();
            score3 = bestBoards.get(2).evaluatePosition();

            if(permutationsAllBoards(piece, piece2, piece3, score1, score2, score3, bestBoards))
                finalResults.add(clone());

            score1 = bestBoards.get(0).evaluatePosition();
            score2 = bestBoards.get(1).evaluatePosition() + offset;
            score3 = bestBoards.get(2).evaluatePosition();
            ...
            score1 = bestBoards.get(0).evaluatePosition();
            score2 = bestBoards.get(1).evaluatePosition();
            score3 = bestBoards.get(2).evaluatePosition() + offset;

            if(permutationsAllBoards(piece, piece2, piece3, score1, score2, score3, bestBoards))
                finalResults.add(clone());

        } else if (isOneLegal && isTwoLegal) {
            score1 = bestBoards.get(0).evaluatePosition() + offset;
            score2 = bestBoards.get(1).evaluatePosition();
        ...
        } else if (isOneLegal && isThreeLegal) {
            score1 = bestBoards.get(0).evaluatePosition() + offset;
            score3 = bestBoards.get(2).evaluatePosition();
        ...
        } else if (isTwoLegal && isThreeLegal) {
            score2 = bestBoards.get(1).evaluatePosition() + offset;
            score3 = bestBoards.get(2).evaluatePosition();
...
//        isGameOver(gameInstances);
    ...
    private boolean permutationsAllBoards(Piece piece, Piece piece2, Piece piece3, int score1, int score2, int score3, ArrayList<Board> bestBoards){
        ...
        if (score1 >= score2 && score1 >= score3) {
            ...
            if(!valuesForNewBoards(score2, score3, newBoardTwoExists, newBoardThreeExists, newBoards))
                return false;
...
//                    System.out.println("1***********************************************GAME OVER ************************************************************************2");
            ...
            if(!valuesForNewBoards(score1, score3, newBoardOneExists, newBoardThreeExists, newBoards))
                return false;
...
//                    System.out.println("2***********************************************GAME OVER ************************************************************************1");
            ...
            if(!valuesForNewBoards(score1, score2, newBoardOneExists, newBoardTwoExists, newBoards))
                return false;
...
//                    System.out.println("3***********************************************GAME OVER ************************************************************************1");
    ...
    private Boolean valuesForNewBoards(int firstScore, int secondScore, boolean firstBoard, boolean secondBoard, ArrayList<Board> newBoards){

        if (newBoards.size() == 2) {
            firstScore = newBoards.get(0).evaluatePosition();
            secondScore = newBoards.get(1).evaluatePosition();
        } else if (firstBoard) {
            firstScore = newBoards.get(0).evaluatePosition();
            secondScore = -1;
        } else if (secondBoard) {
